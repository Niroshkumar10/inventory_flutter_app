...
class ReportService {
  ...
  Future<List<SalesReport>> getSalesReports({
    ...
    try {
      //appLogger.d('📊 Fetching sales reports for user: $userMobile');
      //appLogger.d('📅 Date range: $startDate to $endDate');
      ...
      //appLogger.i('✅ Found ${querySnapshot.docs.length} sales bills');
      ...
      //appLogger.e('❌ Error getting sales reports: $e');
  ...
  Future<List<PurchaseReport>> getPurchaseReports({
    ...
    try {
      //appLogger.d('📊 Fetching purchase reports for user: $userMobile');
      ...
      final querySnapshot = await query.get();
      //appLogger.i('✅ Found ${querySnapshot.docs.length} purchase bills');
      ...
      //appLogger.e('❌ Error getting purchase reports: $e');
      //appLogger.d('Stack trace: $stackTrace');
      rethrow;
    }
  }
  ...
  Future<List<InventoryReport>> getInventoryReports({
    ...
    try {
      //appLogger.d('📊 Fetching inventory reports for user: $userMobile');
      ...
      final querySnapshot = await query.get();
      //appLogger.i('✅ Found ${querySnapshot.docs.length} inventory items');
      ...
      if (status != null && status.isNotEmpty && status != 'All') {
        ...
        //appLogger.i('✅ Filtered to ${reports.length} items with status: $status');
      }

      return reports;
    } catch (e, stackTrace) {
      //appLogger.e('❌ Error getting inventory reports: $e');
      //appLogger.d('Stack trace: $stackTrace');
      rethrow;
    }
  }
  ...
  Future<List<CustomerReport>> getCustomerReports({
    ...
    try {
      //appLogger.d('📊 Fetching customer reports for user: $userMobile');
      ...
      //appLogger.d('📊 Found ${salesQuery.docs.length} sales in period');
      ...
      for (final doc in salesQuery.docs) {
        try {
          ...
          if (customerName.isEmpty) {
            //appLogger.w('⚠️ Bill ${bill.invoiceNumber} has empty customer name');
            continue;
          }
          ...
          //appLogger.w('⚠️ Error processing bill ${doc.id}: $e');
        }
      }
      ...
      //appLogger.i('✅ Generated ${reports.length} customer reports');
      
      // Debug output
      for (final report in reports) {
        //appLogger.d('   👤 ${report.name}: ${report.totalPurchases} purchases, ₹${report.totalSpent}');
      }
      
      return reports;
    } catch (e, stackTrace) {
      //appLogger.e('❌ Error getting customer reports: $e');
      //appLogger.d('Stack trace: $stackTrace');
      rethrow;
    }
  }
...
Future<List<SupplierReport>> getSupplierReports({
  ...
  try {
    //appLogger.d('📊 Fetching supplier reports for user: $userMobile');
    //appLogger.d('📅 Date range: $startDate to $endDate');
    ...
    //appLogger.i('✅ Found ${suppliers.length} suppliers for user: $userMobile');

    if (suppliers.isEmpty) {
      //appLogger.w('⚠️ No suppliers found for this user at all');
      return [];
    }

    // //print ALL suppliers for debugging (not just active)
    for (final supplier in suppliers) {
      //appLogger.d('   👥 Supplier: "${supplier.name}", ID: ${supplier.id}, Active: ${supplier.isActive}');
    }
    ...
    //appLogger.d('📊 Found ${purchasesQuery.docs.length} purchases in period');
    ...
    for (final doc in purchasesQuery.docs) {
      try {
        ...
        if (supplierName.isEmpty) {
          //appLogger.w('⚠️ Purchase bill ${doc.id} has empty supplier name');
          continue;
        }

        //appLogger.d('   🔍 Processing purchase for supplier: "$supplierName"');
        ...
        //appLogger.d('   📝 Purchase details: ₹${bill.totalAmount}, Due: ₹${bill.amountDue}');
      } catch (e) {
        //appLogger.e('❌ Error processing purchase bill ${doc.id}: $e');
      }
    }

    // 3. Show all supplier stats found in purchases
    //appLogger.d('📋 Supplier stats from purchases:');
    for (final entry in supplierStats.entries) {
      //appLogger.d('   📍 "${entry.key}": ${entry.value.totalOrders} orders, ₹${entry.value.totalPurchases}');
    }
    ...
    for (final supplier in suppliers) {
      ...
      for (final entry in supplierStats.entries) {
        ...
        //appLogger.d('   🔄 Comparing bill: "$billSupplierName" with supplier: "$supplierName"');
        
        if (billSupplierName == supplierName) {
          matchingStats = entry.value;
          //appLogger.d('   ✅ Exact match found for supplier: ${supplier.name}');
          break;
        }
      }
      ...
      if (matchingStats == null) {
        for (final entry in supplierStats.entries) {
          ...
          if (billSupplierName.contains(supplierName) || supplierName.contains(billSupplierName)) {
            matchingStats = entry.value;
            //appLogger.d('   🔄 Partial match for supplier: ${supplier.name} -> ${entry.key}');
            break;
          }
        }
      }
    ...
    //appLogger.i('✅ Generated ${reports.length} supplier reports');
    //appLogger.d('📋 Suppliers with purchases: ${reports.where((r) => r.totalPurchases > 0).length}');
    //appLogger.d('📋 Suppliers with zero purchases: ${reports.where((r) => r.totalPurchases == 0).length}');
    
    return reports; // Return ALL suppliers
  } catch (e, stackTrace) {
    //appLogger.e('❌ Error getting supplier reports: $e');
    //appLogger.d('Stack trace: $stackTrace');
  ...
  Future<ProfitLossReport> getProfitLossReport({
    ...
    try {
      //appLogger.d('📊 Calculating P&L for user: $userMobile');
      //appLogger.d('📅 Period: $startDate to $endDate');
      ...
      //appLogger.d('💰 P&L Results:');
      //appLogger.d('   Revenue: ₹$totalRevenue');
      //appLogger.d('   Cost: ₹$totalCost');
      //appLogger.d('   Gross Profit: ₹$grossProfit');
      //appLogger.d('   Expenses: ₹$expenses');
      //appLogger.d('   Net Profit: ₹$netProfit');
      //appLogger.d('   Margin: ${profitMargin.toStringAsFixed(2)}%');
      ...
      //appLogger.e('❌ Error getting profit loss report: $e');
      //appLogger.d('Stack trace: $stackTrace');
      rethrow;
    }
  }
  ...
  Future<List<ReportSummary>> getDashboardSummary({
    ...
    try {
      //appLogger.d('📊 Fetching dashboard summary for user: $userMobile');
      ...
      //appLogger.d('📊 Summary Calculated:');
      //appLogger.d('   Total Sales: ₹$totalSales');
      //appLogger.d('   Total Purchases: ₹$totalPurchases');
      //appLogger.d('   Inventory Value: ₹$totalInventoryValue');
      //appLogger.d('   Outstanding: ₹$totalDue');
      ...
      //appLogger.e('❌ Error getting dashboard summary: $e');
      //appLogger.d('Stack trace: $stackTrace');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> exportReportData({
    ...
    try {
      ...
      //appLogger.e('❌ Error exporting report data: $e');
      //appLogger.d('Stack trace: $stackTrace');
      rethrow;
    }
  }
